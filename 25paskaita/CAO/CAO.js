...
function renderPropertyCards(properties) {
  ...
  properties.forEach((property) => {
    ...
    propertyCard.append(propertyImage, propertyPrice, propertyCity, propertyDescription);
...
function filterProperties(event) {
  if (event.target.tagName.toLowerCase() === "button") {
    ...
    if (isAlreadySelected) {
      ...
      renderPropertyCards(state.properties.filter((property) => property.city === cityClicked));
...
document.getElementById("filters-container").addEventListener("click", filterProperties);
